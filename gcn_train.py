...
def main(data_name, fold):
    ...
    max_count = int (max(user_count,item_count/2+item_count%2))
    ...
    for epoch in range(FLAGS.epochs):
        ...
        feed_dict = construct_feeddict_smodel(user_fea_graph,item_fea_graph,labels,label_weights,placeholders,sparse=False)
       ...
       # t1 = time.time()
        ...
        results += ' '.join(['P@%d:%.6f' % (positions[i], prec[i]) for i in range(len(positions))]) + ' '
        results += ' '.join(['R@%d:%.6f' % (positions[i], rec[i]) for i in range(len(positions))])
        print (results)
    print (1)
