...
class Model(object):
    ...
    def build(self):
        ...
        self.itemneg_activations.append(self.itemneg_inputs)
        for layer in self.item_layers:
            hidden = layer(self.itemneg_activations[-1])
            self.itemneg_activations.append(hidden)
        self.itemneg_outputs = self.itemneg_activations[-1]
        #
...
class NIMC_MF(Model): #mixture of dense and gcn
    def __init__(self, placeholders, user_length, item_length, user_input_dim, item_input_dim, **kwargs):
        ...
        self.itemneg_inputs = placeholders['itemneg_AXfeatures']
    ...
    def _loss(self):
        ...
        for var in self.user_layers[0].vars.values():
            self.loss += FLAGS.weight_decay * 0.001 * tf.nn.l2_loss(var)
